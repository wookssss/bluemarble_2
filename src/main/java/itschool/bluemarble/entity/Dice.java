...
public class Dice {
    ...
    private static int Double_count = 0;



    public int roll() { // 주사위 2개를 굴린다. 2 ~ 12
        if (Double_count == 0) {dice_result = 0;} // 새로운 플레이어가 주사위를 굴릴경우 결과를 0으로 초기화후 주사위 굴리기
        dice1 = 1 + new Random().nextInt(6);
        dice2 = 1 + new Random().nextInt(6);
        dice_result = dice1 + dice2 + dice_result; // 이전에 더블이 있었을 경우 결과 값을 누적
